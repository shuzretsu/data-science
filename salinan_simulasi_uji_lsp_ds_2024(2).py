...
dataset = pd.read_csv("/content/sample_data/breast-cancer-wisconsin_(simulasi-uji-LSP).csv", header=None)
dataset
...
# Dataset describe merupakan informasi info numerik
dataset.describe()
#data describe

import seaborn as sns
import matplotlib.pyplot as plt
#Visualisasi data library
...
# Informasi kostumisasi dari seaborn
...
sample_code_number_counts = dataset ["Sample_code_number"].value_counts()
scn_dict = {
  'SCN yg digunakan hanya 1 kali': 0,
  'SCN yg digunakan 2 kali': 0,
}

for key, count in sample_code_number_counts.items():
  if count > 2:
...
plot_data = dataClean['Class'].replace('4','2').value_counts() # Use parentheses to call the replace method
print(plot_data)
sns.set_theme(font_scale=1.0)
...
# tujuan samplimh untuk menyelaraskan data
...
plt.show()
#ketika dipisah kelasnya, ternyata distribusi kelas sama persis akan terjadi ketidak signifikanan.
...
#bebas perbandingan
#semakin banyak model, semakin bagus
...
#y_test akan di training dengan y_pred
...
# Assuming dataClean is your DataFrame with the features and target variable
X = dataClean.drop('Class', axis=1).values
y = dataClean['Class'].values
...
# Evaluate the Gaussian NB model
print("\nGaussian NB Model:")
a_nb = round(accuracy_score(y_test, y_pred_nb), 3)
print("Accuracy:", a_nb)
print("Classification Report:")
print(classification_report(y_test, y_pred_nb))
evaluation(y_test, y_pred_nb)
...
# Evaluate the Decision Tree model
print("\nDecision Tree Model:")
a_dt = round(accuracy_score(y_test, y_pred_dt), 3)
print("Accuracy:", a_dt)
print("Classification Report:")
print(classification_report(y_test, y_pred_dt))
...
# Evaluate the Random Forest model
print("\nRandom Forest Model:")
a_rf = round(accuracy_score(y_test, y_pred_rf), 3)
print("Accuracy:", a_rf)
print("Classification Report:")
print(classification_report(y_test, y_pred_rf))
evaluation(y_test, y_pred_rf)
